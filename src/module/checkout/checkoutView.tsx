                    ...
                    <div className='lg:col-span-3 border-1 border-gray-500 rounded-t-2xl'>
                        <div className='relative'>
                            ...
                            <div className='py-10 px-20'>
                                <div className='text-2xl font-bold text-center text-violetgradient'>Detail Paket</div>
                                <div className='flex flex-col justify-center items-center lg:flex-row lg:justify-evenly'>
                                    ...
                                    <div className={`text-xl font-semibold mt-4`}>{selectedPackage?.name}</div>
                                    <div className='text-xl font-semibold mt-4'>{numberFormat.format(Number(selectedPackage?.price))}</div>
                                </div>
                                <hr className='my-8 border-violetgradient'/>
                                <div className='text-2xl font-bold text-center text-violetgradient'>Informasi Akun</div>
                                <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 mt-5'>
                                    ...
                                    <div>
                                        ...
                                        />
                                        {isCombine ? <div className='text-sm text-success'>Password Kombinasi</div> : <div className='text-sm text-danger'>Password harus mengandung huruf besar, huruf kecil, angka, dan karakter spesial</div>}
                                    </div>
                                    <div>
                                        <Input 
                                            type="password" 
                                        ...
                                        />
                                        {isMatch ? <div className='text-sm text-success'>Password Cocok</div> : <div className='text-sm text-danger'>Password tidak cocok</div>}
                                    </div>
                                </div>
                    ...
                    <div className="relative  flex flex-col border-1 border-gray-500 rounded-2xl bg-opacity-5">
                        ...
                        {/* Input Column */}
                            <div className="py-10 px-5 flex-grow">
                                {/* <div className='text-2xl font-bold text-center'>Checkout</div> */}
                                <div className="text-xl font-semibold text-violetgradient mt-4">Metode Pembayaran</div>
                                {listPaymentChannel.map((item: any, index: number) => (
                                    <div key={index} className={`border-1 border-gray-500 rounded-3xl px-3 py-4 flex justify-start gap-4 my-4 ${item.code == paymentMethod ? 'bg-blue-600' : ''}`} onClick={
                                        () => {
                                            setPaymentMethod(item.code);
                                        }
                                    }>
                                        <div className='flex flex-col justify-center items-center'>
                                            <Image alt="" className='bg-white p-1 rounded-xl' src={item.icon_url} objectFit='contain' width={80} height={100} />
                                        </div>
                                        <div>
                                            <div className='text-sm font-semibold'>{item.name}</div>
                                            <div className='text-sm'>Biaya Admin: {item.total_fee.flat}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
